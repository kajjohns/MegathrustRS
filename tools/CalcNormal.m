...
function G = Fn(g,rs,cs)
  ...
  for(i = 1:length(cs))
    k = cs(i);
     ...
     [U, D, S] = disloc3d(m, xloc(:,rs), g.mu, .25);
...
%     %calculate tractions
    ...
    %component along dipvec
     ...
     G(:,i) = -S_ten; %negative sign so that positive is compressive stress, consistent with friction law
